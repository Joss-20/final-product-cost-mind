...
export default function Page() {
  ...
  const checkWhoami = async () => { if (token) setWho(await apiJson<Whoami>("/whoami", token)); };
  ...
  useEffect(() => {
    const load = async () => {
      ...
      let q = supabase.from("usage_logs")
  ...
  const { dailyReqs, dailyP95, dailyCost, p95All, total, totalCost } = useMemo(() => {
    if (!metrics.length) return { dailyReqs: [], dailyP95: [], dailyCost: [], p95All: 0, total: 0, totalCost: 0 };
  ...
  return (
    <main className="space-y-8">
      ...
      <section className="card p-5 space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div>
            ...
            <input type="number" min={1} max={50} className="input w-28" value={rows}
              onChange={(e) => setRows(parseInt(e.target.value || "1", 10))} />
          </div>
          <div className="flex-1 min-w-[260px]">
            <div className="label">Feature columns (comma-separated)</div>
            <input className="input w-full" value={featureCols.join(",")}
        ...
        <div className="space-y-2">
          {inputs.map((row, i) => (
            <div key={i} className="flex flex-wrap gap-2">
              {row.map((v, j) => (
                <input key={j} className="input w-28" type="number" value={v}
                  onChange={(e) => {
        ...
        <button className="btn" disabled={!token} onClick={doPredict}>Predict</button>
      ...
      <section className="card p-5 space-y-3">
        ...
        {csvResultUrl && <a className="btn mt-3" download="predictions.csv" href={csvResultUrl}>Download predictions.csv</a>}
      </section>
